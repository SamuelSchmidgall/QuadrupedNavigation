...
def train_sac():
    ...
    agent = SAC(obs_spec, action_spec.shape[0], sac_hyperparams, action_spec)
    ...
    def rand():
       return env.random_state.uniform(action_spec.minimum, action_spec.maximum)
    ...
    for i_episode in range(5000000):
        ...
        episode_reward = 0
        state = env.reset()
        state = generate_obs(state.observation)

        while not done:
            if sac_hyperparams["start_steps"] > total_numsteps:
                action = rand()  # Sample random action
            else:
            ...
            done, reward, _, _s_d = env.step(action)  # Step
            done = done == StepType.LAST
            next_state = generate_obs(sensor_dict=_s_d)
            episode_steps += 1
            total_numsteps += 1
